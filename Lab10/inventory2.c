...
void restore(void) {
    FILE *file;
    char filename[100];
    char line[MAX_NAME_LENGTH + 50]; // Buffer for reading lines
    char name[MAX_NAME_LENGTH];
    int number, quantity;
    ...
    /* Read each line from the file and add to inventory */
    while (fgets(line, sizeof(line), file) != NULL) {
        if (sscanf(line, "%d,%[^,],%d", &number, name, &quantity) == 3) {
            /* Create new node */
    ...
    for (;;) {
        ...
        switch (tolower(code)) {
            ...
            case 'e':
                erase();
                break;
            case 'd': // New operation: dump (save) inventory to file
                dump();
                break;
            case 'r': // New operation: restore (load) inventory from file
                restore();
                break;
            case 'q':
                cleanup();
